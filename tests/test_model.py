...
def test_linear_solver():
  ...
  nx, ny, dt, T, label_dim, traj_num, step_num, U, label = preprocessing(
    arg, type, U, label, device, flag=flag
  )

  print(
    'Checking {} model with n = {}, beta = {:.1f}, Re = {} ...'.format(
      type, n, beta, Re
    )
  )
  ...
  model_ed.load_state_dict(
    torch.load(
      '../models/{}/ED-{}-{}.pth'.format(type, n, ds_parameter),
  ...
  model_ols.load_state_dict(
    torch.load(
      '../models/{}/TR-{}-{}.pth'.format(type, n, ds_parameter),
      map_location=torch.device('cpu')
    )
  )
  model_ols.eval()
  ...
  uv = U[test_index, end:end + batch_size].reshape([batch_size, 2, nx,
                                                    ny]).to(device)
  ...
  #pdb.set_trace()
  loss_ols = criterion(
    outputs2, label[test_index,
                    end:end + batch_size].reshape([label_dim, nx, ny]).to(device)
  )
  ...
  for j in range(0, 1):
    for i in range(0, 1000, batch_size):
      uv = U[test_index, i:i + batch_size].reshape([batch_size, 2, nx,
                                                    ny]).to(device)
      outputs1 = model_ols(uv)
      loss_ols = criterion(
        outputs1,
        label[j, i:i + batch_size, :, :].reshape(batch_size, label_dim, nx, ny)
      )
      outputs2 = model_ed(uv)
      loss_ed = criterion(outputs2, uv)
      print(
        '{}-th AE Loss: {:4e}; traj Loss: {:4e}'.format(
          j, loss_ed.item(), loss_ols.item()
        )
      )
