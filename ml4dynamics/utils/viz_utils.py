...
def plot_1D_spatial_corr(data: list, title: list, fig_name: str):
  for i in range(len(data)):
    corr = calc_utils.calc_1D_spatial_corr(data[i])
    n = len(corr)
    plt.plot(
      np.linspace(-np.pi, np.pi, n), np.roll(corr, n // 2), label=title[i]
    )
  plt.legend()
...
def plot_bias_variance_comparison(data_file: str):
  ...
  def read_data_file(filename):
    ...
    first_moment_biases = [
      data[1, 0], data[1, 1], data[4, 1], data[7, 1], data[10, 1]
    ]
    first_moment_variances = [
      data[1, 2], data[1, 3], data[4, 3], data[7, 3], data[10, 3]
    ]
    second_moment_biases = [
      data[2, 0], data[2, 1], data[5, 1], data[8, 1], data[11, 1]
    ]
    second_moment_variances = [
      data[2, 2], data[2, 3], data[5, 3], data[8, 3], data[11, 3]
    ]
  ...
  methods = [
    'baseline', 'global correction', 'global filter', 'local correction',
    'local filter'
  ]
  assert len(methods) == len(first_moment_biases
                             ) == len(first_moment_variances), "输入列表长度必须一致"
