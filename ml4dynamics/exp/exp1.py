...
def main():
  ...
  def RD_true_model(input):
    ...
    label[:,
          0] = input[:,
                     0] - input[:,
                                0]**3 - input[:,
                                              1] + alpha + 1e-4 * torch.randn(
                                                input[:, 0].shape
                                              )
    label[:, 1] = beta * (input[:, 0] -
                          input[:, 1]) + 1e-4 * torch.randn(input[:, 0].shape)
    return label
  ...
  test_index = 0
  simulator_ablation, _ = test_simulator(
    n=128,
  ...
  simulator_ols, _ = test_simulator(
    n=128,
    test_index=test_index,
    model_type='OLS',
    simu_type=simu_type,
  ...
  for i in range(fig_num):
    ...
    ax2[i].set_axis_off()
    ax3[i].imshow(
      simulator_ablation.u_hist_simu[t_array[i]][x_left:x_right,
                                                 y_left:y_right],
      cmap=cm.jet
    )
    ax3[i].set_axis_off()
  ...
  plt.savefig(
    '../../fig/exp1/{}/1.svg'.format(simu_type),
    dpi=1000,
    bbox_inches='tight',
    pad_inches=0,
    format='svg'
  ...
  plt.savefig(
    '../../fig/exp1/{}/1.pdf'.format(simu_type),
    dpi=1000,
    bbox_inches='tight',
    pad_inches=0
  )
