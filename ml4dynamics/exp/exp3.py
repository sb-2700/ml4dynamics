...
def main():
  ...
  for i in range(5):
    ...
    arg, U, label = read_data(
      '../../data/{}/{}-{}.npz'.format(type, n, ds_parameter[i])
    )

    nx, ny, dt, T, label_dim, traj_num, step_num, U, label = preprocessing(
      arg, type, U, label, device, flag=False
    )

    # loading models
    if type == 'NS':
      model_ols = UNet([2, 4, 8, 32, 64, 128, 1]).to(device)
      u64_np = copy.deepcopy(U[test_index, :,
                               0].numpy()).reshape([step_num, nx + 2, ny + 2])
      v64_np = copy.deepcopy(U[test_index, :, 1, :,
                               1:].numpy()).reshape([step_num, nx + 2, ny + 1])
    else:
      model_ols = UNet([2, 4, 8, 32, 64, 128, 2]).to(device)
      u64_np = copy.deepcopy(U[test_index, :,
                               0].numpy()).reshape([step_num, nx, ny])
      v64_np = copy.deepcopy(U[test_index, :,
                               1].numpy()).reshape([step_num, nx, ny])
    model_ols.load_state_dict(
      torch.load(
        '../../models/{}/OLS-{}-{}.pth'.format(type, n, ds_parameter[i]),
    ...
    model_ed.load_state_dict(
      torch.load(
        '../../models/{}/ED-{}-{}.pth'.format(type, n, ds_parameter[i]),
        map_location=torch.device('cpu')
      )
    )
    model_ed.eval()
    ...
    for j in range(4):
      if i == 0:
        ax.scatter(
          ds_parameter_[i],
          simulator_ols.error_hist[t_array[j]],
          c=c[j],
          marker='o',
          label=r'$t={}$'.format(t_array_[j])
        )
      else:
        ax.scatter(
          ds_parameter_[i],
      ...
      ax1.scatter(
        ds_parameter_[i],
        np.log10(simulator_ols.ds_hist[t_array[j]]),
        c=c[j],
        marker='x'
      )
  ...
  ax.legend()
  plt.savefig(
    '../../fig/exp3/{}/{}Re-ds.pdf'.format(type, type),
    dpi=1000,
    bbox_inches='tight',
    pad_inches=0
  )
