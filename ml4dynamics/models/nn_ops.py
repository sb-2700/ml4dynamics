...
def extract_image_patches(
  lhs, rhs_shape, strides, padding, rhs_dilation, data_format='NHWC'
):
  ...
  lhs_perm = lax.conv_general_permutations((data_format, 'HWIO', data_format)
                                           )[0]
  ...
  dim_num = lax.conv_dimension_numbers(
    lhs.shape, conv_filter.shape, (data_format, 'HWIO', data_format)
  )
  ...
  conv = lax.conv_general_dilated(
    lhs, conv_filter, conv_strides, padding, None, conv_rhs_dilation, dim_num,
    depth
  )
...
def compute_relative_positions(
  query_spatial_shape, key_spatial_shape, spatial_axis=None
):
  ...
  for sa in spatial_axis:
    if not 0 <= sa < len(query_spatial_shape):
      raise ValueError(
        'Element of `spatial_axis` should be between 0 and '
  ...
  relative_positions = np.empty(
    (num_dims, total_queries, total_keys), dtype=np.int32
  )
  ...
  relative_positions = relative_positions.reshape(
    (num_dims, ) + query_spatial_shape + key_spatial_shape
  )
  return relative_positions


def patch_image(
  inputs, inputs_shape, patch_size, strides=None, padding='VALID', mode='i2p'
):
  ...
  if mode == 'i2p':
    ...
    if (inputs_w < patch_w or inputs_h < patch_h):
      raise ValueError(
        f'Patch height and width ({patch_w} and  {patch_h}) '
...
def pooling(
  inputs, window_shape, pooling_configs=None, strides=None, padding='VALID'
):
...
def weighted_avg_pool(
  ...
  pooled_weights = nn.pooling.pool(
    weights, 0., lax.add, window_shape, strides, padding
  )
...
def upscale2x_nearest_neighbor(inputs):
  ...
  output_nchw_shape = (
    -1, input_channels, height_scale * input_h, width_scale * input_w
  )
...
def truncated_normal_initializer(
  stddev: float = 1e-2, dtype: jnp.dtype = jnp.float_
) -> Initializer:
