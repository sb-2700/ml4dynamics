...
from numpy.fft import rfftfreq as nprfftfreq
...
class dynamics(object):
    ...
    def ivp(self, method='RK45'):
        self.sol = solve_ivp(self.f, 
                                t_span=[0, self.T],
                                y0=self.x,
                                method=method,
                                t_eval=np.linspace(0,self.T,self.step_num+1),
                                vectorized=True)
    ...
    def delay_embedding(self, observed_dim=0, method='RK45'):
        ...
        sc = ax.scatter(self.sol.y[observed_dim, :-2], 
                        self.sol.y[observed_dim, 1:-1], 
                        self.sol.y[observed_dim, 2:], c=np.linspace(1,5,self.T), s=np.linspace(1,5,self.T))
        ...
        for t in np.arange(0, T, dt):
            ...
            if int(t/dt+1)%int(T//500/dt) == 0 and self.plot:
                ...
                if int(t/dt+1)%int(T//50/dt) == 0:
                    print("The current numerical value of Lyapunov exp is {:.4f}".format(l_exp/t/dt))

    def calc_attractor_dimension(self):
        epsilon = np.logspace(-2, -1, 10)
        count = np.zeros(epsilon.shape)
        for i in range(epsilon.shape[0]):
            count[i], spanning_set = epsilon_spanning_set(self.sol.y.T, epsilon[i])
        np.savez('../result/dim.npz', T=self.T, dt=self.dt, epsilon=epsilon, count=count)
    ...
    def calc_attractor_dimension2(self, epsilon=0.1):
        ...
        for i in range(sample_num):
            ...
            dim.append(np.log2(nodes_num2/nodes_num1))
        np.savez('../result/dim2.npz', T=self.T, dt=self.dt, epsilon=epsilon, dim=dim)
        return dim
    ...
    def sensitivity_analysis(self, dfds, discretization='CN'):
        ...
        for i in range(n-1):
            if discretization == 'FE':
                ...
                sol[(i+1)*N:(i+2)*N] = (np.eye(N) + dt*J(self.x_hist[:,i])) @ sol[i*N:(i+1)*N] + dfds(self.x_hist[:,i])
            elif discretization == 'BE':
                # backward Euler discretization
                sol[(i+1)*N:(i+2)*N] = solve(np.eye(N) - dt*J(self.x_hist[:,i+1]), sol[i*N:(i+1)*N]+dfds(self.x_hist[:,i+1]))
            elif discretization == 'CN':
                # CN discretization
                sol[(i+1)*N:(i+2)*N] = solve(np.eye(N) - dt/2*J(self.x_hist[:,i+1]), (np.eye(N) + dt/2*J(self.x_hist[:,i])) @ sol[i*N:(i+1)*N]+
    ...
    def lss(self, dt: float, step_num: int, discretization='CN'):
        ...
        for i in range(n-1):
            if discretization == 'FE':
                # forward Euler discretization
                R[i*N:(i+1)*N, (i+1)*N:(i+2)*N] = -torch.eye(N).to(self.device) - dt*J(self.x_hist[...,(n-2-i)*r])
                b[i*N:(i+1)*N] = dfds(self.x_hist[...,(n-2-i)*r])*dt
            elif discretization == 'BE':
                # backward Euler discretization
                R[i*N:(i+1)*N, (i+1)*N:(i+2)*N] = -torch.eye(N).to(self.device)
                R[i*N:(i+1)*N, i*N:(i+1)*N] = R[i*N:(i+1)*N, i*N:(i+1)*N] - dt*J(self.x_hist[...,(n-1-i)*r])
                b[i*N:(i+1)*N] = dfds(self.x_hist[...,(n-1-i)*r])*dt
            elif discretization == 'CN':
                # CN discretization
                R[i*N:(i+1)*N, (i+1)*N:(i+2)*N] = -torch.eye(N).to(self.device) - dt*J(self.x_hist[...,(n-2-i)*r])/2
                R[i*N:(i+1)*N, i*N:(i+1)*N] = R[i*N:(i+1)*N, i*N:(i+1)*N] - dt*J(self.x_hist[...,(n-1-i)*r])/2
                b[i*N:(i+1)*N] = (dfds(self.x_hist[...,(n-1-i)*r])+dfds(self.x_hist[...,(n-2-i)*r]))/2*dt
        ...
        #print('Norm of the residue vector: {:.2e}'.format(nalg.norm(b)))
...
class Rossler(dynamics):
    ...
    def __init__(self, 
                 ...
                 c=14,
                 plot=False):
        super(Rossler, self).__init__(  model_type,
                                        N, 
                                        T, 
                                        dt, 
                                        tol, 
    ...
    def set_attractor(self):
        ...
        if c**2 < 4*a*b:
            ...
            self.attractor = np.zeros(self.N)
            self.attractor[0] = (c+np.sqrt(c**2 - 4*a*b))/2
            self.attractor[1] = -(c+np.sqrt(c**2 - 4*a*b))/2/a
            self.attractor[2] = (c+np.sqrt(c**2 - 4*a*b))/2/a
...
class Lorenz(dynamics):
    ...
    def dfds(self, x):
        # input: x is the 3D state vector of the Lorenz model,
...
class KS(dynamics):
    def __init__(self, 
                 ...
                 c=1,
                 plot=False):
        super(KS, self).__init__(   model_type,
                                    N, 
                                    T, 
                                    dt, 
                                    tol, 
        ...
        dt = self.dt
        k = nprfftfreq(self.N, d=self.L/self.N) * 2 * np.pi
        self.assembly_matrix()
        #print(np.max(np.abs(1 + dt/2 * (k**2) - self.nu*dt/2 * (k**4))/(1 - dt/2 * (k**2) + self.nu*dt/2 * (k**4))))
    ...
    def assembly_matrix(self):
        ...
        for i in range(N):
            ...
            L4[i, (i - 2) % N] = 1
        '''
        # this is for Dirichlet & Neuman boundary condition
        for i in range(N):
            L1[i, (i + 1) % N] = 1
            L1[i, (i - 1) % N] = -1
            L2[i, i] = -2
        ...
        self.L2 = L2/dx**2
        self.L4 = L4/dx**4
        self.Lplus = np.eye(N) + self.c * self.dt/2 * self.L1 + self.dt/2 * self.L2 + self.nu * self.dt/2 * self.L4
        self.Lmimus = np.eye(N) - self.c * self.dt/2 * self.L1 + - self.dt/2 * self.L2 - self.nu * self.dt/2 * self.L4

    def Jacobi(self, x):
        # u_x w term
        jacobi = np.diag(self.L1 @ x) +     \
                    self.L2 + \
                    self.nu * self.L4 + \
                    np.diag(x+self.c) @ self.L1
        return jacobi
    
    def set_attractor(self):
        x_hat = np.zeros(self.N//2+1)
        x_hat[5] = self.N
        x_hat[10] = self.N*2
        x = irfft(x_hat)
        iter = self.CN
    ...
    def CN_FEM_adj(self, x, i):
        ...
        rt = solve(self.Lplus, self.Lmimus@x + self.dt * self.L1@(x*self.x_hist[:,i]) - self.dt * delta_x)
        return rt
    ...
    def set_target_hist(self, x):
        ...
        iter = self.CN
        self.x_targethist = np.zeros([self.N, step_num])
        self.x_hist = np.zeros([self.N, step_num])
        self.y_hist = np.zeros([self.N, step_num])
        self.w_hist = np.zeros([self.N, step_num])
        for i in range(step_num):
            self.x_targethist[:, i] = copy.deepcopy(x)
            x = iter(x)
        self.x_hist[:, 0] = copy.deepcopy(self.x_targethist[:, 0])
    ...
    def calc_gradient(self):
        ...
        for i in range(step_num):
            self.x_hist[:, i] = copy.deepcopy(x)
            #print(nalg.norm(self.x_hist[:, i]-self.x_targethist[:, i]))
            x = iter(x) 
        ...
        #loss = nalg.norm(self.x_hist - self.x_targethist, ord='fro')**2/2/self.N/step_num
        loss = np.mean((self.x_hist - self.x_targethist)**2)/2

        gradient = 0
        bar = 0.5
        # we implement a SGD
        for i in range(step_num):
            #gradient = gradient + np.sum(self.y_hist[:,i]*irfft(rfft(self.x_hist[:,i])*(k**4))) * (1 if (r.rand()>bar) else 0)
            gradient = gradient + np.sum(self.y_hist[:,i]*irfft(rfft(self.x_hist[:,i])*(k**4))) * (1 if (r.rand()>bar) else 0)
    ...
    def calc_init_condition(self):
        ...
        for k in range(iter_num):
            ...
            self.x_hist[:, 0] = self.x_hist[:, 0] - self.y_hist[:, 0]/(i/10+1)/T/self.L*step_size 
    ...
    def calc_gradient_lss(self):
        ...
        self.w_hist_ = np.zeros([self.N, int(T/dt)])
        ...
        loss = nalg.norm(self.x_hist - self.x_targethist, ord='fro')**2/2/self.L/self.T

        gradient = 0
        bar = .5
        # we implement a SGD
        for i in range(int(T/dt)):
            gradient = gradient + np.sum(self.w_hist[:,i]*(self.x_hist[:, i]-self.x_targethist[:, i])) * (1 if (r.rand()>bar) else 0)
            #print(np.sum(self.y_hist[:,i]*irfft(rfft(self.x_hist[:,i])*(k**4))))
...
class NS(dynamics):
    def __init__(self, 
                 ...
                 device=torch.device('cpu'),
                 plot=False):
        super(NS, self).__init__(   model_type,
                                    N, 
                                    T, 
                                    dt, 
                                    tol, 
    ...
    def assembly_spectral(self):
        ...
        k2x = rfftfreq(N*2, d=L/N*2) * 2 * np.pi
        k2y = fftfreq(N*2, d=L/N*2) * 2 * np.pi
    ...
    def CN(self, w_hat):
        ...
        dt = self.dt
        nu = self.nu
        w_hat2 = torch.zeros(w_hat.shape[0]*2, w_hat.shape[1]*2-1, dtype=torch.complex128).to(self.device)
        psi_hat2 = torch.zeros(w_hat.shape[0]*2, w_hat.shape[1]*2-1, dtype=torch.complex128).to(self.device)
        ...
        #print(nalg.norm(wx2*psiy2-wy2*psix2))
        w_hat = ((1 + dt/2 * nu * self.laplacian)*w_hat 
    ...
    def set_x_hist(self, w, iter):
        step_num = self.step_num
        self.xhat_hist = torch.zeros(w.shape[0], w.shape[1], step_num, dtype=torch.complex128).to(self.device)
        self.x_hist = torch.zeros(irfft2(w).shape[0], irfft2(w).shape[1], step_num).to(self.device)
