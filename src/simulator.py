...
class Simulator():
  ...
  def simulator(self, rewrite=False):
    ...
    for i in range(1, self.step_num):
      ...
      self.error_hist[i] = (
        nalg.norm(self.u - self.u_hist[i], 'fro')**2 +
        nalg.norm(self.v - self.v_hist[i], 'fro')**2
      ) / (
        nalg.norm(self.u_hist[i], 'fro')**2 +
        nalg.norm(self.v_hist[i], 'fro')**2
      )
      if self.fluid_type == 'NS':
        self.error_hist[i] = (
          nalg.norm(self.u[1:-1, 1:-1] - self.u_hist[i, 1:-1, 1:-1], 'fro')**2 +
          nalg.norm(self.v[1:-1, :-1] - self.v_hist[i, 1:-1, :-1], 'fro')**2
        ) / (
          nalg.norm(self.u_hist[i, 1:-1, 1:-1], 'fro')**2 +
          nalg.norm(self.v_hist[i, 1:-1, :-1], 'fro')**2
        )
        self.div_hist[i] = np.sum(
          (self.u[1:-1, 1:-1] - self.u[:-2, 1:-1]) / self.dx +
...
class RD_Simulator(Simulator):
  ...
  def __init__(
    ...
    super(RD_Simulator, self
          ).__init__(model, ed_model, device, u_hist, v_hist, step_num, dt, n)
  ...
  def outer_step(self):
    ...
    if self.type == 1:
      ...
      self.u = self.L_uplus @ self.u @ self.L_uplus + self.dt * output[0,
                                                                       0, :, :]
      self.v = self.L_vplus @ self.v @ self.L_vplus + self.dt * output[0,
                                                                       1, :, :]
      ...
      # complex situation
      self.u_ = (
        self.u[::2, ::2] + self.u[::2, 1::2] + self.u[1::2, ::2] +
        self.u[1::2, 1::2]
      ) / 4
      self.v_ = (
        self.v[::2, ::2] + self.v[::2, 1::2] + self.v[1::2, ::2] +
        self.v[1::2, 1::2]
      ) / 4
...
class NS_Simulator(Simulator):
  ...
  def outer_step(self):
    ...
    if ~self.ablation_study:
      ...
      p = sps(self.L, divu.reshape(self.nx * self.ny)).reshape(
        [self.nx, self.ny]
      ) + self.dt * r.randn(self.nx, self.ny) * 1e-5
    ...
    u[:, -1] = -u[:, -2]
    v[0, 1:-1] = 2 * np.exp(
      -50 * (np.linspace(dy, 1 - dy, self.ny - 1) - self.y0)**2
    ) * np.sin(self.t) - v[1, 1:-1]
